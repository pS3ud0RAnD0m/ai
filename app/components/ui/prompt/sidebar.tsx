    ...
    const fetchChatHistory = useCallback(async () => {
        ...
        try {
            ...
            if (selectedProvider === "OpenAI") {
                ...
                const data = await response.json();
                chats = data?.data?.map((chat: { id: string; name: string }) => chat.name) || [];
            }
    ...
    return (
        <div className="mb-4">
                ...
                <Headless.Listbox.Options className="absolute z-10 mt-1 w-full rounded-md text-gray-400 bg-black shadow-lg">
                    {Object.keys(PROVIDERS).map((provider) => (
                        <Headless.Listbox.Option
                            key={provider}
                            value={provider}
                            className={({ active }) => clsx("cursor-pointer select-none px-4 py-2", active ? "bg-green-900 text-gray-400" : "text-gray-400")}
                        >
                            {provider}
                        </Headless.Listbox.Option>
                    ))}
            ...
            <div className="mt-4">
                {chatOption === null ? (
                    <>
                        <button onClick={() => setChatOption("new")} className="w-full text-left px-3 py-2 bg-gray-800 text-gray-300 rounded-md hover:bg-gray-700">
                            New Chat
                        </button>
                        <button
                            onClick={() => {
...
function ModelDropdown({ models, onBack }: { models: string[]; onBack: () => void }) {
    return (
        <div>
            <button onClick={onBack} className="text-gray-400 text-sm underline mb-2">Back</button>
    ...
    return (
        <div>
            ...
            <ul className="bg-gray-800 rounded-md p-2">
                {chatHistory.map((chat, index) => <li key={index} className="py-1 text-gray-300">{chat}</li>)}
            </ul>
        </div>
    );
}