    ...
    >
      ...
      <div className="flex items-center gap-2">
        <Image
          ...
          className="h-6 w-6"
        />
        <NavbarLabel className="text-gray-400 text-lg font-bold">
          AI
        </NavbarLabel>
      </div>
      ...
      <div className="flex items-center gap-16">
        ...
        <NavbarLinkItem
          href="/transformers"
          current={activePath === "/transformers"}
          onClick={() => handleLinkClick("/transformers")}
        >
          Transformers
        </NavbarLinkItem>
        <NavbarLinkItem
...
type NavbarLinkItemProps = React.PropsWithChildren<
  { current?: boolean; onClick?: () => void } & React.ComponentPropsWithoutRef<
    typeof Link
  >
>;
...
export const NavbarLinkItem = React.memo(
  forwardRef<HTMLAnchorElement, NavbarLinkItemProps>(
    (
      { current = false, className, children, href, onClick, ...props },
      ref,
    ) => {
      const classes = clsx(
        "flex items-center gap-2 text-sm font-bold transition",
